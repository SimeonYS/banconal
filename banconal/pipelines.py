...
class BbanconalPipeline:

    # Database setup
    conn = sqlite3.connect('banconal.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `banconal`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM banconal WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO banconal (date, title, link, content)"
